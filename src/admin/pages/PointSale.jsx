...
import { startLoadingInventory } from "../../store/inventario/thunks";
import { ButtonCategory,OrdenProducto,CalculatorOrden,SearchProducto } from "../components/ComponentesPointSale";
import { Producto } from "../components/ComponentesPointSale";

export const PointSale = () => {

  const dispatch = useDispatch();
  const { isLoading, productos,categorias,filtroProductos,filtroProducto } = useSelector(state=>state.inventario);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        if(productos.length > 0){
          dispatch(filterByCategoryPointSale(categorias[0]?.nombre));
        }
      }, [productos]);

  return (
    <div className="overflow-y-auto flex flex-col md:flex-row mr-3 h-full">
      <div className="flex-1 h-full md:overflow-hidden mb-10 md:mb-0">
        <div className="flex flex-col lg:flex-row gap-x-20 items-center">
          <div>
            <h1 className="font-bold text-3xl text-principal">Punto de venta </h1>
            <p className="mt-2">Administra tus ventas de forma facil.</p>
          </div>
          <div className="mt-5 md:mt-0">
            <SearchProducto />
          </div>
        </div>
        <div className="mt-5 h-4/5 w-full">
          <div className="flex gap-3 w-full overflow-auto not-appearance-scrollbar">
            {
              isLoading?
          ...
          <main className="mt-5 h-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-1 md:gap-5 h-4/5 overflow-y-scroll scrollbar overflow-hidden">
              {
              isLoading?
                ...
                // Agregar logica aqui ******************+++
                  ...
                  filtroProductos.map(producto =>(
                    ...
                    <div className="w-full h-full">
                      <h2 className="font-semibold text-center text-principal">No hay productos en esta categoria</h2>
      ...
      {/* facturacion */}
      <section className="flex flex-col p-2 h-full mr-6 lg:mr-0">
        <h1 className="font-bold text-principal text-2xl">Orden actual</h1>
        <div className="flex-1 h-full overflow-hidden md:w-96">
