...
import { ButtonCategory, OrdenProducto } from "../../admin/components/ComponentesPointSale";
import { filterByCategory } from "../../store/inventario/inventarioSlice";
import { startInventoryBusiness } from "../../store/inventario/thunks";
...
import { cleanErrorMessage, cleanSuccessMessage } from "../../store/invoice";
import { clearOrden } from "../../store/orden/ordenSlice";

const Business = () => {

  const location = useLocation();
  const nombreNegocio = location.pathname.split('/')[2]?.toLowerCase().replaceAll('-', ' ');
  const [showModal, setShowModal] = useState({mostrar:false,tipo:''});
  const {successMessage,errorMessage} = useSelector(state=>state.invoice);
  const dispatch = useDispatch();
  const [showOrden, setShowOrden] = useState(false);
  const { isLoading, productos,categorias,filtroProductos,filtroProducto,telefono } = useSelector(state=>state.inventario);

  const { orden,total } = useSelector(state=>state.orden);
    // Realizar thunk si no se ha realizado antes
    useEffect(() => {
      if(categorias.length <= 0){
        dispatch(startInventoryBusiness(nombreNegocio));
      };
    }, []);
    ...
    useEffect(() => {
      if(successMessage !== null && successMessage){
          ...
          MySwal.fire({
            icon: 'success',
            html: <i className="text-blue-500 font-semibold">El precio puede variar por el tipo de servicio...</i>,
  ...
  useEffect(() => {
    ...
    // Primer renderizado carguen productos
    useEffect(() => {

      if(productos.length > 0){
        dispatch(filterByCategory(categorias[0]?.nombre));
      }
    }, [productos]);
    
  return (
    <main className="w-full h-full">
      <div className="w-full m-auto relative h-full">
        <div className="fixed md:absolute w-14 right-1 md:right-5 bottom-1/4 md:bottom-0 cursor-pointer transition ease-in-out hover:-translate-y-1 hover:scale-110 duration-300
        "
        onClick={()=>setShowOrden(!showOrden)}
        >
            <img className="w-full" src="https://res.cloudinary.com/djvsdtbda/image/upload/v1676059933/journal/iek91hcw12ordd64v7ag.png" alt="Carrito de compras" />
        </div>
        <h1
          className=" text-center font-extrabold text-transparent mb-3 text-4xl sm:text-6xl bg-clip-text bg-gradient-to-r from-sky-400 to-blue-800"
        >
          {nombreNegocio?.toUpperCase()}
        </h1>
        {
          telefono!=''&&
          <aside className="flex items-center justify-center gap-x-4 mb-5 mt-4">
        <a className="text-blue-500 font-bold flex items-center gap-2 mb-2" href={`https://wa.me/+593${telefono}/?text=Hola,%20necesito%20mas%20informacion%20sobre%20su%20negocio%20que%20vi%20en%20la%20pagina%20web%20de%20capifood.com`} target="_blank" >
      ...
      <div className="m-auto flex gap-3 w-4/5 overflow-auto not-appearance-scrollbar">
        {
          isLoading?
            ...
            categorias.map((categoria,index) =>(
              <ButtonCategory categoria={categoria} key={index} />
            ))
        }
      </div>
      <section className={`mt-5 h-full w-4/5 m-auto ${showOrden&&'hidden'}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-5 h-full">
          {
          isLoading?
      ...
      <section className={`${showOrden?'absolute items-center h-full w-full top-0 bottom-0 z-20 bg-white':'hidden'}`}>
        ...
        <form className="bg-primario p-4 flex flex-col gap-4 w-full sm:w-4/5 sm:mx-auto">
          ...
          <div className="flex justify-between gap-x-2 md:gap-x-5">
            <button disabled={orden?.length == 0} onClick={()=>setShowModal({mostrar:true,tipo:'Para llevar'})} type={"button"} className="md:w-full bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-3 border-b-4 border-blue-700 hover:border-blue-500 rounded">
              Para llevar
            </button>
            <button disabled={orden?.length == 0} onClick={()=>setShowModal({mostrar:true,tipo:'A domicilio'})} type={"button"} className="md:w-full bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-3 border-b-4 border-blue-700 hover:border-blue-500 rounded ">
              A domicilio
            </button>
          </div>
      </form>
      </section>
      <ModalVenta showModal={showModal} setShowModal={setShowModal} setShowOrden={setShowOrden}></ModalVenta>
    </main>
  )
}
